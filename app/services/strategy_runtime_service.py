            ...
            with self.db.session() as session:
                record = StrategyRunModel(
                    ...
                    load_forecast_json=json.dumps([
                        ...
                        for p in (load_forecast.points if load_forecast else [])
                    ]) if load_forecast else None,
                    pv_forecast_json=json.dumps([
                        {"timestamp": p.timestamp.isoformat(), "value": p.value_kw}
                        for p in (pv_forecast.points if pv_forecast else [])
                    ]) if pv_forecast else None,
                    schedule_json=json.dumps([
                        {"timestamp": p.timestamp.isoformat(), "power": p.storage_power_target_kw, "soc": p.predicted_soc}
                        for p in (optimization.schedule if optimization else [])
                    ]) if optimization and optimization.schedule else None,
                )
                session.add(record)
                session.commit()
