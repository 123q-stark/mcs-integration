...
# ===== B-P0-06: 使用 ModeLiteral 强校验 =====
ModeLiteral = Literal["AUTO", "PV_PRIORITY", "ECONOMIC_SCHEDULE", "GRID_BACKUP", "SAFE"]
...
class StrategyConfigUpdate(BaseModel):
    ...
    # ===== 新增字段（B-01）使用 ModeLiteral =====
    requested_mode: ModeLiteral = Field(..., description="请求模式: AUTO/PV_PRIORITY/ECONOMIC_SCHEDULE/GRID_BACKUP/SAFE")
    ...
    # requested_mode 的校验由 ModeLiteral 自动完成，无需手动 validator
...
class ModeUpdate(BaseModel):
    """模式更新请求模型（B-P0-06: 使用 ModeLiteral 强校验）"""
    requested_mode: ModeLiteral = Field(..., description="请求模式: AUTO/PV_PRIORITY/ECONOMIC_SCHEDULE/GRID_BACKUP/SAFE")


# ============ 设备策略配置 Schema（B-P0-06 新增） ============

class StrategyDeviceConfigUpdate(BaseModel):
    """设备策略配置更新请求模型（支持部分更新）"""
    participate_in_strategy: bool | None = None
    allow_strategy_control: bool | None = None
    strategy_power_limit_kw: float | None = Field(None, ge=0, description="策略功率上限 (kW)")
    priority: int | None = Field(None, ge=1, le=5, description="优先级 1-5")